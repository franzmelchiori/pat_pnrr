...
FOLDER_COMUNI_EXCEL = 'pat_pnrr_9a_misurazione_tabelle_comunali\\'  # pat_pnrr_importazione_tabelle_comunali\\'
DATA_INIZIO_MONITORAGGIO = '2025' + '-07-01'  # '-01-01'
DATA_FINE_MONITORAGGIO = '2025' + '-12-31'  # '-06-30'
PERIODO_MONITORAGGIO = '2025q3-4'
CODICE_MONITORAGGIO = 'mpe_09'
INDEX_COMUNI_EXCEL_MAP = 7
    ...
    def get_comune_dataframe(self, sheet_name):
        ...
        if sheet_name == 'Prov di sanatoria':
            ...
            if comune_dataframe.loc[:, 'data_inizio_pratica'].dtype.str[1] in ['O', 'M']:
                ...
                change_mask = comune_dataframe.loc[:, 'data_inizio_pratica'].astype(
                    'string').str.contains('8/4/20285', case=False, na=False, regex=False)
                comune_dataframe.loc[change_mask, 'data_inizio_pratica'] = '08/04/2025'
                change_mask = comune_dataframe.loc[:, 'data_inizio_pratica'].astype(
                    'string').str.contains('ARCHIVIATA', case=False, na=False, regex=False)
                comune_dataframe.drop(comune_dataframe[change_mask].index, inplace=True)
            try:
        ...
        if sheet_name == 'Controllo CILA':
            ...
            if comune_dataframe.loc[:, 'data_fine_pratica'].dtype == 'O':
                ...
                comune_dataframe.drop(comune_dataframe[change_mask].index, inplace=True)
                change_mask = comune_dataframe.loc[:, 'data_fine_pratica'].astype(
                    'string').str.contains('Rifiutata', case=False, na=False, regex=False)
                comune_dataframe.drop(comune_dataframe[change_mask].index, inplace=True)
                change_mask = comune_dataframe.loc[:, 'data_fine_pratica'].astype(
                    'string').str.contains('Ritirata', case=False, na=False, regex=False)
                comune_dataframe.drop(comune_dataframe[change_mask].index, inplace=True)
...
if __name__ == '__main__':
    ...
    get_comuni_measures(comuni_excel_map, save_tex=False, tsf=True)  # stampa 8 misure per tutti i ped da tutti i comuni
